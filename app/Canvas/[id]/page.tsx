...
import { Canvas } from "@/app/data/productdata"; // Data file se Canvas array import kiya
...
export default function CanvasDetailPage() {
  ...
  // Canvas data array mein se product match karna
  const product = Canvas.find((item) => item.id === productId);

  if (!product) {
    return (
      <div style={{ padding: "40px", textAlign: "center", fontFamily: "sans-serif" }}>
        <h2>Product Not Found!</h2>
        <Link href="/Canvas" style={{ color: "#0070f3" }}>Go back to Canvas Collection</Link>
      </div>
    );
  }

  // WhatsApp Configuration (Apna real active number country code ke sath likhein)
  ...
  return (
    <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 20px", fontFamily: "sans-serif" }}>
      ...
      <Link href="/Canvas" style={{ textDecoration: "none", color: "#666", display: "inline-block", marginBottom: "30px", fontWeight: "500" }}>
        ← Back to Canvas Collection
      </Link>
