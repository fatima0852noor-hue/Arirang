// app/Calligraphy/[id]/page.tsx
"use client";
import { useParams } from "next/navigation";
import { Calligraphy } from "@/app/data/productdata"; 
import Link from "next/link";

export default function ProductDetailPage() {
  const params = useParams();
  
  // URL se ID lekar numeric check match karne ke liye convert kar rahe hain
  const productId = Number(params.id);

  // Calligraphy data array mein se select kiya gaya single product match karna
  const product = Calligraphy.find((item) => item.id === productId);

  // Agar ghalat ID open ho to screen block error handle karna
  if (!product) {
    return (
      <div style={{ padding: "40px", textAlign: "center", fontFamily: "sans-serif" }}>
        <h2>Product Not Found!</h2>
        <Link href="/Calligraphy" style={{ color: "#0070f3" }}>Go back to Calligraphy</Link>
      </div>
    );
  }

  // WhatsApp Configuration (Apna real active number mention karein country code ke sath)
  ...
  return (
    <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 20px", fontFamily: "sans-serif" }}>
      ...
      {/* Back Button Line Link */}
      <Link href="/Calligraphy" style={{ textDecoration: "none", color: "#666", display: "inline-block", marginBottom: "30px", fontWeight: "500" }}>
        ← Back to Calligraphy Collection
      </Link>

      <div style={{ display: "flex", gap: "50px", flexWrap: "wrap" }}>
        
        {/* Left Layout Grid: Product Image Only */}
        ...
        {/* Right Layout Grid: Text Data Fields */}
        <div style={{ flex: "1 1 450px", display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          {/* WhatsApp Direct Order Button Redirect */}
          <a 
            href={whatsappUrl} 
            target="_blank" 
          ...
          >
            {/* WhatsApp SVG Visual Element Icon Wrapper */}
